...
3 條獨立滾軸**永不停下**、各自速度不同，blur 到看不清單張臉。reels 之間的相對位置持續漂移，**偶爾**撞進 45 種預設連線之一 — 撞到那一 frame 就畫線（桃色 payline + 3 格光暈），沒撞到就維持 blur，**每 frame 等時、無 hold**。

- **每一 frame snap 到整數 tile**（沒有滑到一半的狀態）→ 觀眾在 FB 點暫停，任何一 frame 都看到 9 張完整對齊的臉
- **3 條不同步進速率**：reel 0 = 1 frame/tile（fps=15 時 67ms，strobe blur）、reel 1 = 2 frame/tile、reel 2 = 3 frame/tile
- **startPos 由 45 種預設連線隨機抽一個當種子**，保證 f=0 是一條真實的連線（符合「至少有一條連線」的承諾）
- **cycle 內其他 frame 若剛好撞到 45 種之一也會畫線**（漂移帶來的意外連線），沒撞到就不畫
- 拉霸 LCM 週期 = 54 frame @ fps=15，一個 cycle 裡通常有 1–4 個連線 frame
- 用戶 FB 點暫停：大部分時候看到「3 條 blur 不同進度」、~2–8% 機率剛好撞到連線 frame（中獎）
- **無減速尾段、無 hold、無 pulse**：連線 frame 跟其他 frame 視覺權重完全一樣 → 保留「點到哪停哪」的 slot machine 抽籤感
...
- **字母標籤已拿掉（2026-09-07）**：早期每格前面掛 `[A]..[I]` 當對位錨點，
  靠 OUTPUT RULES「不要畫任何文字 / 字母 / 數字」壓著不讓它畫出來。換到
  gemini-3.1-flash-image 之後，照片參考圖會把九個字母烤進每格角落（動漫圖不會）。
  現在只留方位詞 `top-left cell → …`，方位詞從來沒被畫進圖裡；實測九格指定
  不同天氣，位置 9/9 全對，沒有退化。
...
- [x] 更多影格策略 → v2「拉霸滾輪 3×3」blur spin 永不停，LCM 週期內偶爾撞到 45 種預設連線才畫線，保留隨機暫停感
